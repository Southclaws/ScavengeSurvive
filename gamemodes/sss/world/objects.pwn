...
Load_Objects()
{
	DirectoryCheck(DIRECTORY_SCRIPTFILES DIRECTORY_MAPS);
	DirectoryCheck(DIRECTORY_SCRIPTFILES DIRECTORY_MAPS DIRECTORY_SESSION);
	...
	if(gDebugLevel > DEBUG_LEVEL_NONE)
		Logger_Log("object loader debug", Logger_I("level", gDebugLevel));
	...
	{
		Logger_Log("added objects", Logger_I("amount", gTotalLoadedObjects));
		Logger_Log("removed objects", Logger_I("amount", gTotalObjectsToRemove));
	}
...
hook OnPlayerDisconnect(playerid, reason)
{
	...
	if(gDebugLevel >= DEBUG_LEVEL_INFO)
		printf("[OnPlayerDisconnect] Removing session data file for %s", name);
...
RemoveObjects_FirstLoad(playerid)
{
	...
	if(gDebugLevel >= DEBUG_LEVEL_INFO)
		printf("[RemoveObjects_FirstLoad] Created session data for %s", name);
	...
	for(new i; i < gTotalObjectsToRemove; i++)
	{
		...
		if(gDebugLevel >= DEBUG_LEVEL_DATA)
			printf("[RemoveObjects_FirstLoad] Write: [%x.%x.%x.%x.%x]", buffer[0], buffer[1], buffer[2], buffer[3], buffer[4]);
...
RemoveObjects_OnLoad(playerid)
{
	...
	{
		if(gDebugLevel >= DEBUG_LEVEL_INFO)
			printf("[RemoveObjects_OnLoad] Session data for %s doesn't exist, running firstload.", name);
	...
	if(gDebugLevel >= DEBUG_LEVEL_INFO)
		printf("[RemoveObjects_OnLoad] Loading removals for %s", name);
	...
	for(new i; i < gTotalObjectsToRemove; i++)
	{
		...
		if(gDebugLevel >= DEBUG_LEVEL_DATA)
			printf("[RemoveObjects_OnLoad] Append: [%x.%x.%x.%x.%x]", buffer[0], buffer[1], buffer[2], buffer[3], buffer[4]);
