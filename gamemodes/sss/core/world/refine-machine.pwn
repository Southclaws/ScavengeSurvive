...
{
	dbg("global", CORE, "[OnPlayerConnect] in refine-machine");
...
{
	dbg("global", CORE, "[OnPlayerUseMachine] in refine-machine");

	if(GetItemType(itemid) == item_RefineMachine)
	{
		dbg("refine-machine", 1, "[OnPlayerUseMachine] itemid %d interactiontype %d", itemid, interactiontype);
...
_rm_PlayerUseRefineMachine(playerid, itemid, interactiontype)
{
	dbg("refine-machine", 1, "[_rm_PlayerUseRefineMachine] playerid %d itemid %d interactiontype %d", playerid, itemid, interactiontype);
	...
	if(GetItemTypeLiquidContainerType(itemtype) != -1)
	{
		...
		{
			dbg("refine-machine", 1, "[_rm_PlayerUseRefineMachine] starting HoldAction for %ds starting at %ds", floatround(MAX_REFINE_MACHINE_FUEL), floatround(data[rm_fuel]));
...
{
	dbg("global", CORE, "[OnItemAddToContainer] in refine-machine");
...
{
	dbg("global", CORE, "[OnHoldActionUpdate] in refine-machine");

	if(rm_CurrentRefineMachine[playerid] != -1)
	{
		dbg("refine-machine", 3, "[OnHoldActionUpdate] Refine machine itemid %d progress %d", rm_CurrentRefineMachine[playerid], progress);
		...
		if(GetItemTypeLiquidContainerType(GetItemType(itemid)) != -1)
		{
			...
			{
				dbg("refine-machine", 3, "[OnHoldActionUpdate] Stopping HoldAction: player not holding petrol can");
		...
		if(fuel <= 0.0)
		{
			dbg("refine-machine", 3, "[OnHoldActionUpdate] Stopping HoldAction: petrol can has %d < 0 fuel", fuel);
			StopHoldAction(playerid);
			rm_CurrentRefineMachine[playerid] = -1;
		...
		else
		{
			...
			dbg("refine-machine", 3, "[OnHoldActionUpdate] setting petrol can to %f, machine to %f", fuel - 1.1, machinefuel + 1.1);
...
_rm_StartCooking(itemid)
{
	...
	dbg("refine-machine", 1, "[_rm_PlayerUseRefineMachine] itemid %d", itemid);
	...
	dbg("refine-machine", 2, "[_rm_PlayerUseRefineMachine] itemcount %d cooktime %ds", itemcount, cooktime);
	...
	dbg("refine-machine", 2, "[_rm_PlayerUseRefineMachine] started cooking...");
...
#pragma unused cooktime
	dbg("refine-machine", 1, "[_rm_PlayerUseRefineMachine] finished cooking...");
	new data[e_REFINE_MACHINE_DATA];
	...
	for(new i = GetContainerItemCount(containerid) - 1; i > -1; i--)
	{
		...
		dbg("refine-machine", 3, "[_rm_PlayerUseRefineMachine] slot %d: destroying %d", i, subitemid);
