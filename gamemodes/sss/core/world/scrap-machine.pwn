...
{
	dbg("global", CORE, "[OnPlayerConnect] in scrap-machine");
...
{
	dbg("global", CORE, "[OnPlayerUseMachine] in scrap-machine");

	if(GetItemType(itemid) == item_ScrapMachine)
	{
		dbg("scrap-machine", 1, "[OnPlayerUseMachine] itemid %d interactiontype %d", itemid, interactiontype);
...
_sm_PlayerUseScrapMachine(playerid, itemid, interactiontype)
{
	dbg("scrap-machine", 1, "[_sm_PlayerUseScrapMachine] playerid %d itemid %d interactiontype %d", playerid, itemid, interactiontype);
	...
	if(GetItemTypeLiquidContainerType(itemtype) != -1)
	{
		...
		{
			dbg("scrap-machine", 1, "[_sm_PlayerUseScrapMachine] starting HoldAction for %ds starting at %ds", floatround(MAX_SCRAP_MACHINE_FUEL), floatround(Float:data[sm_fuel]));
...
{
	dbg("global", CORE, "[OnHoldActionUpdate] in scrap-machine");

	if(sm_CurrentScrapMachine[playerid] != -1)
	{
		dbg("scrap-machine", 3, "[OnHoldActionUpdate] Scrap machine itemid %d progress %d", sm_CurrentScrapMachine[playerid], progress);
		...
		if(GetItemTypeLiquidContainerType(GetItemType(itemid)) != -1)
		{
			...
			{
				dbg("scrap-machine", 3, "[OnHoldActionUpdate] Stopping HoldAction: player not holding petrol can");
		...
		if(fuel <= 0.0)
		{
			dbg("scrap-machine", 3, "[OnHoldActionUpdate] Stopping HoldAction: petrol can has %d < 0 fuel", fuel);
			StopHoldAction(playerid);
			sm_CurrentScrapMachine[playerid] = -1;
		...
		else
		{
			...
			dbg("scrap-machine", 3, "[OnHoldActionUpdate] setting petrol can to %f, machine to %f", fuel - 1.1, machinefuel + 1.1);
...
_sm_StartCooking(itemid)
{
	...
	dbg("scrap-machine", 1, "[_sm_PlayerUseScrapMachine] itemid %d", itemid);
	...
	dbg("scrap-machine", 2, "[_sm_PlayerUseScrapMachine] itemcount %d cooktime %ds", itemcount, cooktime);
	...
	dbg("scrap-machine", 2, "[_sm_PlayerUseScrapMachine] started cooking...");
...
#pragma unused cooktime
	dbg("scrap-machine", 1, "[_sm_PlayerUseScrapMachine] finished cooking...");
	new data[e_SCRAP_MACHINE_DATA];
	...
	for(new i = GetContainerItemCount(containerid) - 1; i > -1; i--)
	{
		...
		dbg("scrap-machine", 3, "[_sm_PlayerUseScrapMachine] slot %d: destroying %d", i, subitemid);
	...
	for(new i; i < scrapcount; i++)
	{
		...
		dbg("scrap-machine", 3, "[_sm_PlayerUseScrapMachine] Created item %d and added to container %d", subitemid, containerid);
	}
