...
public OnLoad()
{
	...
	CaveDoor = CreateButton(-2702.358398, 3801.477050, 52.652801, "Press F to enter", 0, 1); // cave 1

	buttonid[0] = CreateButton(-2811.1840, -1524.0532, 140.8437, "Press F to enter", 0, 1);
	CreateDoor(1497, buttonid,
		-2811.41, -1524.75, 139.84,   0.00, 0.00, 455.82,
		-2811.41, -1524.72, 139.84,   0.00, 0.00, 570.18, .maxbuttons = 1, .movespeed = 0.1, .interiorid = 1);

	buttonid[0] = CreateButton(-2821.0671, -1518.6484, 140.8437, "Press F to enter", 0, 1);
	CreateDoor(1497, buttonid,
		-2821.19, -1519.46, 139.84, 0.00, 0.00, 450.60,
		-2821.19, -1519.44, 139.84, 0.00, 0.00, 353.28, .maxbuttons = 1, .movespeed = 0.1, .interiorid = 1);


	buttonid[0] = CreateButton(-2796.933349, 3682.779785, 02.515481, "Press F to enter", 0, 1); // cave 1
	buttonid[1] = CreateButton(-785.9272, 3727.1111, 0.5293, "Press F to enter", 0, 1); // cave 2
	LinkTP(buttonid[0], buttonid[1]);

	// Subway/Metro interior 1=inside 2=surface

	buttonid[0] = CreateButton(-1007.395263, 5782.741210, 42.951477, "Press F to climb up the ladder", 0, 1);
	buttonid[1] = CreateButton(2526.719482, -1648.620605, 14.471982, "Press F to climb down the ladder");
	LinkTP(buttonid[0], buttonid[1]);

	buttonid[0] = CreateButton(250.599380, -154.643936, -50.768798, "Press F to enter", 0, 1);
	buttonid[1] = CreateButton(247.878799, -154.444061, 02.399550, "Press F to enter");
	LinkTP(buttonid[0], buttonid[1]);

	buttonid[0] = CreateButton(-2276.608642, 5324.488281, 41.677970, "Press F to enter", 0, 1);
	buttonid[1] = CreateButton(-734.773986, 3861.994628, 12.482711, "Press F to enter", 0, 1); // cave
	LinkTP(buttonid[0], buttonid[1]);

	// Fort Claw underground

	buttonid[0]=CreateButton(246.698684, -178.849655, -50.199367, "Press F to enter", 0, 1); // underground
	buttonid[1]=CreateButton(-952.559326, 5137.799804, 46.183383, "Press F to enter", 0, 1); // metro station
	LinkTP(buttonid[0], buttonid[1]);

	CreateButton(-972.153869, 4303.185058, 48.666248, "~r~Locked", 0, 1);

	// Lift Sequence

	CaveLiftButtonT=CreateButton(-2764.0332, 3757.0466, 46.8343, "Press F to use the lift", 0, 1);
	CaveLiftButtonB=CreateButton(-2764.3410, 3755.5153, 8.2390, "Press F to use the lift", 0, 1);
	LiftPos=0;

	// Fort Claw Door

	buttonid[0] = CreateButton(264.316284, -171.135223, -50.206447, "Press F to activate", 0, 1);
...
public OnButtonPress(playerid, buttonid)
{
	...
	if(buttonid == QuarryDoor)
	{
	    ...
	    {
	    	ShowPlayerDialog(playerid, d_NULL, DIALOG_STYLE_MSGBOX, "Door", "You pull on the door but it won't budge, the lock seems sturdy.\nThere's no way you can get through here without a key.\nPerhaps you should search the shed?", "Close", "");
	    }
	    else
	    {
