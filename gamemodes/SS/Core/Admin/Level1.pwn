...
new gAdminCommandList_Lvl1[] =
{
	...
	/checksob - check if player is using Sobiet."
};
...
ACMD:checksob[1](playerid, params[])
{
    new targetid;
    if(sscanf(params, "u", targetid))
        return Msg(playerid,YELLOW," > Usage: /checksob [ PlayerName / PlayerID ]");

    TogglePlayerControllable(targetid, false);
    defer RunSobietCheckerForPlayer(playerid, targetid);
    return true;
}

timer RunSobietCheckerForPlayer[4000](playerid, targetid)
{
    new Float:vector[ 3 ], PlayerName[ MAX_PLAYER_NAME ];
    GetPlayerCameraFrontVector(targetid, vector[ 0 ], vector[ 1 ], vector[ 2 ]), GetPlayerName(targetid, PlayerName, MAX_PLAYER_NAME);
