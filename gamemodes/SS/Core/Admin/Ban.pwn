...
hook OnGameModeInit()
{
	...
	stmt_BanGetFromNameIp		= db_prepare(gAccounts, "SELECT COUNT(*), "FIELD_BANS_DATE", "FIELD_BANS_REASON", "FIELD_BANS_DURATION" FROM "ACCOUNTS_TABLE_BANS" WHERE "FIELD_BANS_NAME" = ? AND "FIELD_BANS_ACTIVE"=1 COLLATE NOCASE OR "FIELD_BANS_IPV4" = ? ORDER BY "FIELD_BANS_DATE" DESC");
	stmt_BanNameCheck			= db_prepare(gAccounts, "SELECT COUNT(*) FROM "ACCOUNTS_TABLE_BANS" WHERE "FIELD_BANS_NAME" = ? AND "FIELD_BANS_ACTIVE"=1 COLLATE NOCASE ORDER BY "FIELD_BANS_DATE" DESC");
