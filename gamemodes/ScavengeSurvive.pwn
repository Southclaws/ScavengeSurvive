...
new gMessageOfTheDay[MAX_MOTD_LEN],
...
// INTERNAL
new gBigString[MAX_PLAYERS][4096], gTotalRules, gTotalStaff;
...
OnGameModeInit_Setup()
{
    ...
    {
        printf(
            "UNKNOWN ERROR: gBuildNumber is below 1000: %d this should never happen! Ensure you've cloned the repository correctly.",
            gBuildNumber
        );
        for (; ; )
        {}
    ...
    {
        print(
            "ERROR: ./scriptfiles directory detected using old directory structure, please see release notes for stable release #04"
        );
        for (; ; )
        {}
    }
    ...
    {
        print(
            "ERROR: Directory '" DIRECTORY_SCRIPTFILES DIRECTORY_MAIN "' not found. Creating directory."
        );
    ...
    if (gServerMaxUptime > 0)
    {
        ...
        if (gServerUptime >= gServerMaxUptime - 3600)
        {
            ...
            format(
                str,
                36,
                "Server Restarting In:~n~%02d:%02d",
                (gServerMaxUptime - gServerUptime) / 60,
                (gServerMaxUptime - gServerUptime) % 60
            );
...
{
    new query[96], DBResult:result, dbcolumns;
